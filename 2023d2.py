...
for line in A.splitlines():
    ...
    R.append(game)
    valid = True
    draws = [dict([reversed(amt.split()) for amt in draw.split(', ')]) for draw in D.split('; ')] 
    C = defaultdict(list)
    for draw in draws:
        for color, amount in draw.items():
            C[color].append(int(amount))
    S += product({c: max(v) for c, v in C.items()}.values()) 
    for draw in draws:
        for color, amount in draw.items():
            if int(amount) > L[color]:
                #print(f"game {game} not valid, {amount} {color}")
                R = R[:-1]
                valid = False
                break
        if not valid:
