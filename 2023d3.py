...
for y, line in enumerate(A.splitlines()):
    #print([el for el in re.split('\.+', line) if el])
    obj=None
    for x, c in enumerate(line):
        if obj is None and c!='.':
            if c.isdigit():
                obj=c
                start = x, y
            else:
...
for obj in objs:
    ...
    for sym in symbols:
        s, x, y = sym
        if sx-1 <= x <= ex+1 and sy-1 <= y <= ey+1:
            #print(obj, sym)
            if s == '*':
                G[(x, y)].append(int(num)) 
            P.append(int(num)) 
            added=True
    if not added:
        print(obj)
...
print(sum(P))
print(R)
#for a, b in [(78,2),(23,1),(12,4), (1,2),(2,2), (90,1), (56,1), (34,1)]: 
#    assert P.count(a) == b, f"{a}: expect {b}, actual {P.count(a)}"
#assert sum(P) == 413
