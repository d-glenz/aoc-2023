...
class Field:
    ...
    def neighbors(self, pos):
        ...
        if x > 0:
            l = self.data[y][x-1], (x-1,y)
        if y > 0:
            t = self.data[y-1][x], (x, y-1)
        if y < len(self.data)-1:
            b = self.data[y+1][x], (x, y+1)
        if x < len(self.data[0])-1:
            r = self.data[y][x+1], (x+1,y)
...
def next_pos(f, pos, typ=None):
    x, y = pos
    if typ is None:
        typ=Pipe(f[pos]) 
    t, l, b, r = f.neighbors(pos)
    match typ:
        case Pipe.ns:
            return t[1], b[1]
        case Pipe.we:
...
def traverse_path(f):
    ...
    f[f.start] = st.value
    path = Path()
    path.path1.append((f.start, st.value)) 
    path.path2.append((f.start, st.value)) 
